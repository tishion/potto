...
POTTO_ERROR CreateInstance(const PottoUuid& classId, const PottoUuid& interfaceId, void** ppv,
                           const std::string& moduleName) {
  // Query the IClassFactory interface
  PottoPtr<IClassFactory> pClassFactory;
  POTTO_ERROR error =
      GetClassObject(classId, IIDOF(IClassFactory), (void**)&pClassFactory, moduleName);
...
POTTO_ERROR GetClassObject(const PottoUuid& classId, const PottoUuid& interfaceId, void** ppv,
                           const std::string& moduleName) {
  // Validate
  if (!ppv)
  ...
  if (moduleName.empty()) {
    ...
    if (POTTO_E_OK != error) {
      const std::string modulePath =
          PottoModulePoolManager::GetInstance().GetModulePathByClassId(classId);
      if (modulePath.empty())
        return error;
